...
//     deltaX = eleSpac / 4 / 20 * *steerLabel;
    ...
    plhs[0] = mxCreateNumericMatrix(n1 * m1 * (sampleSize - lenFilt + 1), 1,  mxSINGLE_CLASS, 0);
...
//     
    ...
    if (NULL == ( filtData0 = (float *)malloc(sizeof(float) * m1 * n1 * sampleSize) ) )
    {
...
//                     if (i * n1 * sampleSize + j * sampleSize + j1 >=  m1 * n1 * sampleSize)
//                     {
//                         printf("error\n");
//                     }
//                     else
//                     {
                    filtData0[i * n1 * sampleSize + j * sampleSize + j1] += 
