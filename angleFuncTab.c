...
//     deltaX = eleSpac / 4 / 20 * *steerLabel;
    ...
    plhs[0] = mxCreateNumericMatrix(n1, m1,  mxINT32_CLASS, 0);
    ape = (int *) mxGetPr(plhs[0]);
    plhs[1] = mxCreateNumericMatrix(n1 * m1 * 65, 1,  mxSINGLE_CLASS, 0);
    delay = (float *)mxGetPr(plhs[1]);
    plhs[2] = mxCreateNumericMatrix(n1 * m1 * 65, 1,  mxSINGLE_CLASS, 0);
    apeWin = (float *)mxGetPr(plhs[2]);
    ...
    for (i = 0; i < m1; i ++)      // x-direction
    {
        ...
        for (j = 0; j < n1; j ++)  // z-direction
        {
             ...
             for (k = -apeSize; k < apeSize + 1; k ++)
             {
                 ...
                 // delay = ;
                 // Uint in [s]
                 delay[(i * n1 + j) * 65 + k + apeSize] = (z + trx)/c * fs + tTot;
                 //mexPrintf("%d\n", delay);
                 if (*delay >= (m - 1))
                     continue;
                 
                 
                 apeWin[(i * n1 + j) * 65 + k + apeSize] = 0.54f - 0.46f * cosf(pi/apeSize*(k + apeSize));
               //   dasData[i * n1 + j] = j;
