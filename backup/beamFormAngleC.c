...
//     deltaX = eleSpac / 4 / 20 * *steerLabel;
    ...
    //
    eleSpac = 0.3048e-3;
    sapSpac = eleSpac/2/sin(fabs(alphaR))/20;
...
//     #pragma omp parallel for shared(dasData, rawData, alphaR, eleSpac, steerLabel, n, m, deltaZ, deltaX, pi) \
    ...
    for (i = 0; i < m1; i ++)      // x-direction
    {
        ...
        for (j = 0; j < n1; j ++)  // z-direction
        {
             ...
             for (k = -apeSize; k < apeSize + 1; k ++)
             {
               ...
               //   dasData[i * n1 + j] = j;
                 dasData[i * n1 + j] += (0.54 - 0.46 * cos(pi*(k + apeSize + 0.0)/apeSize))     \
                                 * ( (delay - (int) (delay))                                  \
